    ...
    <table class="table">
        ...
        @foreach($data as $datas)
            <tr>
                ...
                <th>{{ $datas->name }}</th>
                <td>
                    <form action="{{ route('kecamatan.destroy', $datas->id) }}" method="POST">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
                        <a href="#" name="btnEdit" id="btnEdit" data-toggle="modal" data-target="#exampleModal" data-names="{{$datas->name}}" data-kabupaten="{{$datas->kabupaten_kota_id}}" data-ids="{{$datas->id}}" class="btn btn-sm btn-primary">Edit</a>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('kecamatan.update', 1) }}" method="POST">
                    {{ csrf_field() }}
                    {{ method_field('PUT') }}
                    <input type="hidden" readonly="readonly" class="form-control id" name="ids" required="required">
                    <div class="form-group">
                        <label for="kabupaten">ID Kabupaten Kota</label>

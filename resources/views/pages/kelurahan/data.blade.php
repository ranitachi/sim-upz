    ...
    <table class="table">
        ...
        @foreach($data as $datas)
            <tr>
                ...
                <th>{{ $datas->name }}</th>
                <td>
                    <form action="{{ route('kelurahan.destroy', $datas->id) }}" method="POST">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
                        <a href="#" name="btnEdit" id="btnEdit" data-toggle="modal" data-target="#exampleModal" data-names="{{$datas->name}}" data-kecamatan="{{$datas->kecamatan_id}}" data-ids="{{$datas->id}}" class="btn btn-sm btn-primary">Edit</a>
